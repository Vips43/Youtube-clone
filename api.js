...
/**=======================================
   global search api
 *=======================================*/
export async function getData(q = 'vipul gehlot') {
...
export async function channel() {
  ...
  const chnl = channels.map(c => c.channel);
  console.log(chnl);

  return youtube.channelData = chnl.map(ch => ({
...
export async function channelRelResult() {
  ...
  youtube.channelRelResult = info.map(i => ({
    ...
    authUrl: i.video.author.canonicalBaseUrl || '',
    authTitle: i.video.author.title || '',
    channelId: i.video.author.channelId || '',
    videoId: i.video.videoId || '',
    vTitle: i.video.title || '',
    ...
    HQ: i.video.thumbnails?.[1]?.url || i.video.thumbnails?.[0]?.url || '',
